...
public class PgScoreDao implements ScoreDao {
	private static final String SQL_INSERT_SCORE = "INSERT INTO scores (login_id, score, difficulty) VALUES(:login_id, :score, :level)";
	...
	public List<Score> findAll(Integer level) {
		String SQL_SELECT_ALL = "SELECT RANK() OVER(ORDER BY score DESC) AS lank, login_id, score FROM (SELECT s.login_id, score FROM scores s JOIN users u ON s.login_id = u.login_id WHERE s.difficulty = :level) a LIMIT 30";
